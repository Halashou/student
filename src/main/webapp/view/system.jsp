...
<head>
    ...
    <script type="text/javascript">
	 var _menus = {"menus":[
						...
						{"menuid":"4","icon":"","menuname":"年级专业管理",
							"menus":[
									{"menuid":"62","menuname":"年级专业列表","icon":"icon-house","url":"GradeServlet?method=toGradeListView"}
								]
						},
						</c:if>
						<c:if test="${userType == 1 || userType == 3}">
...
<body class="easyui-layout" style="overflow-y: hidden"  scroll="no">
    ...
    <div region="north" split="true" border="false" style="overflow: hidden; height: 30px;
        ...
        <span style="float:right; padding-right:20px;" class="head"><span style="color:red; font-weight:bold;">
        <c:if test="${sessionScope.userType eq '1' or userType eq '3'}"> ${user.teachername}</c:if>
        <c:if test="${userType eq '2' }">${user.studentname }</c:if>
        &nbsp;</span>您好&nbsp;&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/LoginServlet?method=logout" id="loginOut">安全退出</a></span>
        <span style="padding-left:10px; font-size: 16px; ">学生信息管理系统</span>
    </div>
    <div region="south" split="true" style="height: 30px; background: #D2E0F2; ">
        <div class="footer">Copyright &copy; 蒲公英工作室 2019. Powered By LiANG</div>
    </div>
	...
	<iframe width="0" height="0" src="${pageContext.request.contextPath}/refresh.jsp"></iframe>
